          ...
          <div className="mx-auto mt-8 max-w-4xl space-y-5">
            ...
            {failedUploads.length ? (
              <div className="rounded-[28px] border border-amber-100 bg-amber-50/80 p-4 text-sm text-amber-800 shadow-sm">
                {failedUploads.length} file{failedUploads.length > 1 ? "s" : ""} hit an error.
                You can retry by dropping them in again.
              </div>
            ) : null}

            <ProcessingStatus uploads={uploads} />
            <ResultsList files={results} isPolling={isPolling} />

            {!uploads.length && !results.length ? (
            ...
            <div className="flex justify-center">
              <button
                className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white/90 px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm hover:-translate-y-0.5 hover:border-slate-300 hover:bg-white"
